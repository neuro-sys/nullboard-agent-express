...
app.get("/board/:id", (req, res) => {
  ...
  // prefer meta.json's "current" pointer - it's what the client considers active,
  // and is the only thing updated when undo/redo moves to an older revision
  // without rewriting any rev-*.nbx file (so mtime alone can't detect that move)
  ...
  if (metaRaw) {
    try {
      ...
      const candidate = `rev-${meta.current}.nbx`;
      if (revisionFiles.includes(candidate)) currentFile = candidate;
    } catch (_) {
      // fall through to mtime-based fallback below
    }
  }

  const latestFile =
    currentFile ||
    revisionFiles.reduce((latest, file) =>
